...
def train_loop(model, device, train_loader, test_loader, optimizer, model_type='teacher', teacher_model=None, tensor_mask=None):
    ...
    for epoch in tqdm(range(30)):
        ...
        #print(f"Norm List {layer_norm}, Mean Norm {mean_norm} Test Acc {test_acc}")
        scheduler.step()
...
def main():
    ...
    model_save_path=glob.glob(os.path.join('saved_models', 'mnist', '*'))
    train_loader, test_loader = utils.dataloader('mnist')
    
    model_list=[Base(), BaseWide(), BaseDropout(), BasePReLU(), LeNet5(), LeNet5Dropout()]

    for model_index, model in enumerate(model_list):
        model_save_path=os.path.join('saved_models', 'mnist', model.name)
        ...
        student_data=datasets.EMNIST('../data', split='letters', download=True,
        ...
        if split:
            ...
            student_train_loader=DataLoader(student_train_data_subset, batch_size=32)
            student_test_loader=DataLoader(student_test_data_subset, batch_size=32)
        else:
            student_train_loader=Dataloader(student_data, batch=32)
        print("Training Student Model on Random Data of length ", len(student_train_loader.dataset))

        optimizer = optim.Adam(filter(lambda p: p.requires_grad, student_model.parameters()), lr=1e-3)
        ...
        if not os.path.exists(os.path.join('recovery_data', 'mnist', 'no_data_10', 'drop', model.name.split('.')[0])):
            os.makedirs(os.path.join('recovery_data', 'mnist','no_data_10',  'drop',  model.name.split('.')[0]))
        np.save(os.path.join('recovery_data', 'mnist','no_data_10',   'drop', model.name.split('.')[0], 'rad_test_acc_list'), rad_test_acc_list)
        np.save(os.path.join('recovery_data', 'mnist', 'no_data_10', 'drop',  model.name.split('.')[0], 'mean_norm'), mean_norm_list)
        np.save(os.path.join('recovery_data', 'mnist', 'no_data_10', 'drop',  model.name.split('.')[0], 'layer_norm_list'), layer_norm_list)
