...
def main():
   ...
   #         tensor_mask.append(mask)
    ...
    for (name, param), mask in zip(student_model.named_parameters(), tensor_mask):
        for num, sm_param in enumerate(student_model.parameters()):
            sm_param.requires_grad=False

        param.requires_grad=True

        optimizer = optim.Adam(filter(lambda p: p.requires_grad, student_model.parameters()), lr=1e-5)
